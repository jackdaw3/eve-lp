...
func (o *OfferSerivce) convertOffer(offer *model.Offer) (*dto.OfferDTO, error) {
	...
	if err != nil {
		offerDTO.Error = true
		errorMessage := fmt.Sprintf("failed to get %s price for <b>%s</b>: %s",
...
func (o *OfferSerivce) conertMaterials(rs model.RequireItems, offerDTO *dto.OfferDTO) dto.MatertialDTOs {
	...
	for _, r := range rs {
		...
		if err != nil {
			offerDTO.Error = true
			errorMessage := fmt.Sprintf("failed to get %s price for requirement <b>%s</b>: %s",
				o.materialPrice,
...
func (o *OfferSerivce) conertManufactMaterials(ms model.ManufactMaterials, offerDTO *dto.OfferDTO) dto.MatertialDTOs {
	...
	for _, m := range ms {
		...
		if err != nil {
			offerDTO.Error = true
			errorMessage := fmt.Sprintf("failed to get %s price for production material <b>%s</b>: %s",
				o.materialPrice,
				material.MaterialName,
				errors.Cause(err).Error(),
			)
